...
function problemC () {
  ...
  promisifiedReadFile('poem-one/stanza-02.txt')
    .then((stanza2) => {
      blue(stanza2)
      // faltaba retornar dentro del then
...
function problemD () {
  ...
  .catch(function errorHandler(err) { magenta(new Error(err))} )
  .catch((err) => {return magenta(new Error(err))})
  .finally(() => {console.log('done')})
...
function problemE () {
  ...
  // aca sabemos que no hay otro poema por eso podriamos no ponerlo
  // .then((stanza4) => {blue(stanza4)})
  .catch((err) => {return magenta(new Error(err))})
